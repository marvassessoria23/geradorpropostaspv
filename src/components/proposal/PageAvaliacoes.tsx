...
const PageAvaliacoes: React.FC<Props> = ({ data, pageNumber, bgColor, onChange }) => {
  const up = onChange || (() => {});

  const updateAv = (id: string, field: string, value: string) => {
    up({ avaliacoes: data.avaliacoes.map((a) => (a.id === id ? { ...a, [field]: value } : a)) });
  };

  const visibleAvs = data.avaliacoes.filter((a) => !(a as any).hidden);
  ...
  return (
    <div className="slide" data-slide style={{ padding: '48px 64px', backgroundColor: bgColor || '#e8c96a' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(visibleAvs.length, 2)}, 1fr)`, gap: 20 }}>
        {visibleAvs.map((av) => (
          <div key={av.id} style={{ background: '#ffffff', borderRadius: 12, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <div>
                <InlineEditable value={av.nome} onChange={(v) => updateAv(av.id, 'nome', v)} style={{ fontFamily: "'Lato', sans-serif", fontWeight: 700, color: '#0d2b45', fontSize: 14, margin: 0, display: 'block' }} />
            ...
            <InlineEditable
              value={av.texto}
              onChange={(v) => updateAv(av.id, 'texto', v)}
              multiline
              style={{ fontFamily: "'Lato', sans-serif", color: '#1a3a5c', fontSize: 13, lineHeight: 1.6, margin: 0, display: 'block' }}
            />
          </div>
        ))}
      </div>
